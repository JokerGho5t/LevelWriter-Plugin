...
struct LEVELWRITER_API FLWEventConfig
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, meta = (ToolTip = "AND - ALL conditions must be met\nOR - AT LEAST ONE"))
	ELWLogicOperand ConditionOperand = ELWLogicOperand::AND;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "If true, the script will run only once. To restart, you need call ResetDoOnce function"))
	bool bExecuteOnce = false;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Sequential - actions execute one by one\nParallel - all execute at once"))
	ELWExecutionMode ExecutionMode = ELWExecutionMode::Sequential;

	UPROPERTY(EditAnywhere, meta = (EditCondition = "ExecutionMode == ELWExecutionMode::Parallel", EditConditionHides, ToolTip = "ALL - waits for all actions to finish\nANY - finishes when at least one action finishes (interrupting others)"))
	ELWParallelMode ParallelMode = ELWParallelMode::ALL;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Number of times to repeat the actions\n-1 - infinite loop\n1 - single execution"))
	int32 LoopCount = 1;
};
